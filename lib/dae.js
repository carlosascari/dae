...
class Dae extends Daemon {
  constructor() {
    ...
    this.scannerRef = setInterval(() => {
      ...
      this.scanning = true;
      this.scan().then(() => this.scanning = false);
    }, SCANNER_INTERVAL);
  }
  ...
  scan(options) {
    return new Promise((resolve, reject) => {
      ...
      .then((stats) => {
        ...
        const candidates = stats.filter(stat => stat.pid).filter(stat => stat.pid != process.pid);

        if (candidates.length === 0) {
          resolve();
          return console.log(`scan - No candidate Daemons found`);
        }
        ...
        .then((results) => {
          ...
          if (daemonCredentials.length === 0) {
            resolve();
            return console.log(`scan - No valid Daemons found`);
          }
          ...
          daemonCredentials.forEach(daemonCredential => {
            ...
            if (daemonCredential.name === realCredential.name) {
              ...
              console.log('[Dae] found: (daemon: `%s` pid: %s port: %s)', realCredential.name, realCredential.pid, realCredential.port)
              console.log('.')
              found.push(realCredential);
            } else {
              console.log('[Dae] found: {COPY CAT}=', daemonCredential, realCredential);
            }
          });
          resolve(found);
        })
